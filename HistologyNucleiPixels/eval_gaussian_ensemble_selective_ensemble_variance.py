...
for run_i in range(num_runs):
    ...
    for i in range(M):
        ...
        network = GaussNet(model_id + "_eval_gaussian_ensemble_selective_ensemble_variance", project_dir="/root/regression_uncertainty/HistologyNucleiPixels").cuda()
        network.load_state_dict(torch.load("/root/regression_uncertainty/HistologyNucleiPixels/training_logs/model_%s_%d/checkpoints/model_%s_epoch_%d.pth" % (model_id, model_i, model_id, epoch)))
    ...
    print ("sigma_epi_test_pred_props:")
    print (sigma_epi_test_pred_props)
    print ("sigma_epi_test_pred_props: %g +/- %g" % (np.mean(np.array(sigma_epi_test_pred_props)), np.std(np.array(sigma_epi_test_pred_props))))
    print ("$")
    print ("$")
    print ("sigma_epi_test_coverages:")
    print (sigma_epi_test_coverages)
    print ("sigma_epi_test_coverages: %g +/- %g" % (np.mean(np.array(sigma_epi_test_coverages)), np.std(np.array(sigma_epi_test_coverages))))
    print ("$")
    print ("$")
    print ("sigma_epi_test_avg_lengths:")
    print (sigma_epi_test_avg_lengths)
    print ("sigma_epi_test_avg_lengths: %g +/- %g" % (np.mean(np.array(sigma_epi_test_avg_lengths)), np.std(np.array(sigma_epi_test_avg_lengths))))
    print ("###################################################################")
    print ("###################################################################")
    print ("###################################################################")
    print ("###################################################################")
