...
for run_i in range(num_runs):
    ...
    for i in range(M):
        ...
        network = GaussNet(model_id + "_eval_ensemble", project_dir="/root/regression_uncertainty/HistologyNucleiPixels").cuda()
    ...
    for step, (xs, ys) in enumerate(val_loader):
        with torch.no_grad():
            ...
            sigma_2 = torch.zeros(preds[0].size()).cuda()
            for value in preds:
                sigma_2 = sigma_2 + torch.pow(pred - value, 2)/M_float

            preds = pred
            sigmas = torch.sqrt(sigma_2)
    ...
    lower_values = pred_values - scipy.stats.norm.ppf(1.0 - alpha/2)*sigma_values
    upper_values = pred_values + scipy.stats.norm.ppf(1.0 - alpha/2)*sigma_values
