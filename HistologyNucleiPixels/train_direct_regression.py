...
for i in range(num_models):
    ...
    network = GaussNet(model_id + "_%d" % i, project_dir="/root/regression_uncertainty/HistologyNucleiPixels").cuda()
    ...
    for epoch in range(num_epochs):
        ...
        for step, (xs, ys) in enumerate(train_loader):
            ...
            preds, _ = network.head_net(x_features) # (shape: (batch_size, 1))
            preds = preds.view(-1) # (shape: (batch_size))

            ########################################################################
            # compute loss:
            ########################################################################
            loss = torch.mean(torch.pow(ys - preds, 2))
